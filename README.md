...
- `Xiaoqing-blog-api/`：后端子模块（跟踪 `main` 分支）
- `Xiaoqing-blog-web/`：前端子模块（跟踪 `main` 分支）
- `deploy/`
  - `deploy/backend/Dockerfile`：后端镜像构建
  - `deploy/nginx/Xiaoqing-blog.conf`：Nginx 反向代理示例
...
- Nginx 配置示例：[deploy/nginx/Xiaoqing-blog.conf](deploy/nginx/Xiaoqing-blog.conf)
...
```text
/srv/nimbus/                       # 仓库根目录（上传）
  Xiaoqing-blog-api/                 # 后端子模块
    config.yaml                    # 迁移专用配置：仅包含 PostgreSQL（host=localhost）
    migrations/                    # 迁移 SQL
    dist/                          # 上传的二进制（如 dist/Xiaoqing-blog-api、dist/migrate）
  Xiaoqing-blog-web/                 # 前端子模块
  deploy/                          # 部署文件
    backend/Dockerfile
    nginx/Xiaoqing-blog.conf
    infra/docker-run.sh
...
file_storage:
  provider: minio
minio:
  external_base_url: https://<服务器域名>/minio
```
...
因此在服务器上建议准备目录：`/srv/nimbus/Xiaoqing-blog-api/`，保证该目录下同时存在 `config.yaml` 与 `migrations/`，并在该目录执行迁移命令。
...
set GOARCH=amd64
set CGO_ENABLED=0
cd Xiaoqing-blog-api
mkdir dist
go build -o dist\Xiaoqing-blog-api .\cmd\migrate
```

上传到服务器：

- 上传 `dist/migrate` 到 `/srv/nimbus/Xiaoqing-blog-api/dist/`

#### 4.2) 执行迁移

在目录 `/srv/nimbus/Xiaoqing-blog-api/` 执行迁移：
...
后端镜像构建依赖二进制文件：`Xiaoqing-blog-api/dist/Xiaoqing-blog-api`（见 `deploy/backend/Dockerfile` 的 `COPY` 指令）。
...
- 上传 `dist/Xiaoqing-blog-api` 到 `/srv/nimbus/Xiaoqing-blog-api/dist/`
...
```bash
sudo docker build -f deploy/backend/Dockerfile -t Xiaoqing-blog-api:latest .
```
...
上传到服务器 Xiaoqing-blog-web/：`.next`、`public`、`package.json`、`pnpm-lock.yaml`

在服务器安装依赖并启动前端（在 `Xiaoqing-blog-web/` 内执行）：

```bash
cd /srv/nimbus/Xiaoqing-blog-web
...
```bash
sudo cp deploy/nginx/Xiaoqing-blog.conf /etc/nginx/sites-available/Xiaoqing-blog.conf
sudo ln -s /etc/nginx/sites-available/Xiaoqing-blog.conf /etc/nginx/sites-enabled/Xiaoqing-blog.conf
```
